...
/**
 * Controller class that defines the behaviour of the Editor menu
 * application. This is the entry point of all the applications.
 * @class Editor
 * #depends EditorWindow.js
 * #depends view/EditorMessages.js
 * #depends view/RecordingPromotionItem.js
 * #depends view/MusicPromotionItem.js
 * #depends view/EditorIconTextItem.js
 * @static
 * @param {Object} $N namespace entry point
 */
var Editor = (function ($N) {
	var log = new $N.apps.core.Log("Editor", "Editor"),
		view = {},
		menu = null,
		PVRSubscriptionIds = [],
		...
		dataMapper = {
			getTitle: function (obj) {
				return Editor.getString(obj.title);
			},
	...
	/**
	 * Returns the list of active options available in the Editor menu
	...
	function EditorUpdate(selectedItem) {
		menu.displayData(true, true); // TODO: combine with redrawEditor()
	}

	function subscribeMailStatusChangedEvent() {
		if (!mailStatusChangedSubID) {
			mailStatusChangedSubID = $N.apps.util.EventManager.subscribe("allMailsRead", EditorUpdate, this);
		}
	}
	...
	/**
	 * @method refreshEditorWindowData
	 * @private
	 */
	function refreshEditorWindowData() {
		...
		if (selectedItem && selectedItem.itemHandlerData.refreshOnFocus) {
			...
			selectedItem.itemHandlerData.getData(mode, function (data) {
				if (data && $N.apps.core.ContextManager.getActiveContext().getId() === "Editor") {
					EditorWindow.refreshData(data);
				}
			});
		}
	}
	...
	function subscribeOnTasksChangedEvent() {
		window.parent.CCOM.Scheduler.addEventListener("onTasksChanged", refreshEditorWindowData);
	}
	...
	function unSubscribeOnTasksChangedEvent() {
		window.parent.CCOM.Scheduler.removeEventListener("onTasksChanged", refreshEditorWindowData, false);
	}
	 ...
	 * @method restoreEditorSelection
	 * @param {Object} menuItem : item to be highlighted.
	 * @param {boolean} isSelect : true will activate the Editor window, false will just preview.
	 * @param {Number} currentPreviewIndex : previously selected cell index.
	 * @private
	 */
	function restoreEditorSelection(menuItem, isSelect, currentPreviewIndex) {
		log("restoreEditorSelection", "Enter");
		var activationObject;
		if (menuItem) {
			activationObject = {
				mode: menuItem.itemHandlerData.activationObject.mode
			};
			EditorWindow.unPreview();
			if (menuItem.itemHandlerData.getData) {
				menuItem.itemHandlerData.getData(activationObject.mode, function (data) {
					if (data) {
						...
						if (isSelect) {
							menu.defocus();
							EditorWindow.preview(activationObject);
							isWindowSelected = true;
							if (currentPreviewIndex !== null) {
								EditorWindow.getCurrentTable().setRootCellIndex(currentPreviewIndex);
							}
							EditorWindow.activate(activationObject);
						} else {
							EditorWindow.preview(activationObject);
						}
					}
		...
		log("restoreEditorSelection", "Exit");
	}
	...
	/**
	 * redraws the Editor menu
	 * highlight is brought back to the previously highlighted component
	 * @method redrawEditorMenu
	 * @private
	 */
	function redrawEditorMenu() {
		log("redrawEditorMenu", "Enter");
		var dvrMenuItemIndex = findMenuItemIndex("menuDVR"),
			itemHighlightIndex = 0,
			currentMenuItemIndex = 0,
			currentPreviewIndex = null,
			EditorWindowTable = EditorWindow.getCurrentTable(),
			selectedItem = menu.getSelectedItem(),
			currentlySelectedItemTitle = (selectedItem) ? selectedItem.title : null,
			activeMenuItems;

		//currentPreviewIndex gets back the index of the Editor Window Table that is currently highlighted.
		if (EditorWindowTable && EditorWindowTable.getController()) {
			currentPreviewIndex = EditorWindowTable.getController().getSelectedCellIndex();
		}
		...
		if (!currentlySelectedItemTitle || currentlySelectedItemTitle !== menu.getSelectedItem().title) {
			//menu item gets previewed or activated based on Editor Window selection.
			if (isWindowSelected) {
				restoreEditorSelection(activeMenuItems[itemHighlightIndex], true, currentPreviewIndex);
			} else {
				restoreEditorSelection(activeMenuItems[itemHighlightIndex], false);
			}
		}
		log("redrawEditorMenu", "Exit");
	}
	...
	function subscribeToPVRCapabilityEvent() {
		if (PVRSubscriptionIds.length === 0) {
			...
			PVRSubscriptionIds = $N.app.PVRCapability.subscribeToPVRCapabilityEvent(PVRStatusUpdateListener, PVRStatusUpdateListener, Editor);
		}
	}
	...
	function subscribeToUsbMediaPlaybackEvent() {
		if (PlaybackSubscriptionIds.length === 0) {
			// we are adding these subscribe events to an array as we do not specifically need to know the Ids
			PlaybackSubscriptionIds = $N.app.UsbBrowserHelper.subscribeToUsbMediaPlaybackEvent(usbMediaPlaybackListener, usbMediaPlaybackListener, Editor);
		}
	}
	/**
		...
		if (!isOKPressed) {
			...
			case "MEDIABROWSER":
				...
				if (activationObject.mode === 'dlna') {
					...
					$N.app.DlnaHelper.showDeviceList(dlnaOptionSelectedCallback, Editor.getString("dlnaPopUpTitle"), Editor.getString("dlnaPopUpMessage"));
					return;
				}
				break;
			}
	 ...
	 * When the callback is fired, Editor menu gets redrawn
	...
	return {
		...
		load: function () {
			...
			$N.apps.core.Language.importLanguageBundleForObject(Editor, Editor.init, "apps/Editor/common/", "LanguageBundle.js", null, window);
			log("load", "Exit");
		},
		...
		init: function () {
			...
			$N.apps.core.ContextManager.initialisationComplete(Editor);
		...
		keyHandler: function (key, repeats) {
			...
			if (isWindowSelected) {
				handled = EditorWindow.keyHandler(key);
			} else {
				handled = menu.keyHandler(key);
			}
			if (!handled) {
				switch (key) {
				case keys.KEY_MENU:
					...
					if (pinTimer) {
						...
						$N.app.ContextHelper.openContext("ZAPPER", {
							activationContext: {
								keyHandlerKey: key,
								showBanner: true,
								isDirectChannelEntryFromEditor: true
							}
						});
					}
					handled = true;
